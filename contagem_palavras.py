...
def write_to_csv(all_texts_folder_source, target_file, grade):
                ...
                if(
                    ...
                    if(
                        ...
                        if (dicio.search(token.text) == None):
                            print('incorreta', token)
                            # print('incorreta', token.text)
    ...
    with target_file:
        ...
        writer.writerow({
            'Série': grade,
            'Quantidade de textos': files_count,
            'Número de palavras': words_count,
            'Número de palavras incorretas': misspelled_word_count,
        })
    # encerra a execução
...
def main(grade):
    ...
    # builds target file
    target_file_name = f"{str(grade)}_grade_words_count.csv"
